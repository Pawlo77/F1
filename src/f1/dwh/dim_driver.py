...
class DimDriver(Base, DWHMixin):
    ...
    driver_name = Column(
        String(100),
        nullable=False,
        comment="Display name of the driver. From f1db.driver",
    )
    driver_first_name = Column(
        String(100), nullable=False, comment="Driver's first name. From f1db.driver"
    )
    driver_last_name = Column(
        String(100), nullable=False, comment="Driver's last name. From f1db.driver"
    )
    driver_full_name = Column(
        String(100), nullable=False, comment="Driver's full name. From f1db.driver"
    )
    driver_abbreviation = Column(
        String(3),
        nullable=False,
        comment="Driver abbreviation (typically 3 letters). From f1db.driver",
    )
    driver_permanent_number = Column(
        String(2),
        nullable=True,
        comment="Driver's permanent racing number. From f1db.driver",
    )
    driver_gender = Column(
        String(6), nullable=False, comment="Gender of the driver. From f1db.driver"
    )

    driver_date_of_birth = Column(
        Date, nullable=False, comment="Date of birth. From f1db.driver"
    )
    driver_date_of_death = Column(
        Date, nullable=True, comment="Date of death (if applicable). From f1db.driver"
    )
    driver_place_of_birth = Column(
        String(100), nullable=False, comment="Place of birth. From f1db.driver"
    )
